...
## window (smooth), then idle, then battery lowers idle further. The player's
## 30 FPS cap limits everything.


## state: {"minimized": bool, "interacting": bool, "on_battery": bool,
##         "cap_30": bool}. config: data/settings/performance.json.
...
static func validate(config: Dictionary) -> Array[String]:
	...
	for key in ["interactive_fps", "capped_fps", "idle_fps", "battery_fps", "minimized_fps"]:
